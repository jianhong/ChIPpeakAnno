...
peakcallwithinput <- function(input.sample.file,input.bam.file,genome) {
  ...
  cellInfo.run <- lapply(1:length(cellInfo),function(u,cellInfo,temp3){
    ...
    x.run<-apply(x.sample,1,function(x){
      ...
      file.name <- y[5]
       ...
       cmd12=paste(cmd9,xx,"-c",x.input,cmd10,paste0(temp3,"/",paste0(xx.name,"_hs_1.00e-05_macs142_2")),cmd11,sep=" ")
      # 
       cmd12
      #print(cmd12)
    })
  ...
  names(cellInfo.run)=unlist(lapply(cellInfo,function(u){u$name}))
