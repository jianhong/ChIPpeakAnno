GFF2RangedData <- function(data.GFF,header=FALSE, ...)
{
	...
	{
		stop("No valid data.GFF passed in, which is a data frame or file as GFF format with 9 required fields. Please refer to http://genome.ucsc.edu/FAQ/FAQformat#format3 for details. If it is a file, please make sure the file path is correct!")
	}
	if (header == TRUE)
	{	
		myPeak = data.GFF[-1,]
	...
	{
		stop("only 1, -1, +, - and . are acceptable for strand info. Please double check you GFF data. If strand entries contain '.', please convert it into '+' or '-'.")
	}
	strand[strand== "1"] = "+"
	strand[strand=="-1"] = "-"
