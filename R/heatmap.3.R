## modified function based on heatmap.2.R in gplots
...
)
{
  ...
  {
    flag <- try(
      stats:::plot.dendrogram(ddr, horiz = TRUE, axes = FALSE, yaxs = "i", leaflab = "none")
    )
  ...
  {
    flag <- try(
      stats:::plot.dendrogram(ddc, axes = FALSE, xaxs = "i", leaflab = "none")
    )
    if("try-error" %in% class(flag))
    {
      cond <- attr(flag, "condition")
  ...
  if(key)
  {
    ...
    z <- as.vector(x)[order(as.vector(x))]
    ...
    image(z=matrix(z, ncol=1),
          col=col, breaks=tmpbreaks,yaxt="n",xaxt="n")
