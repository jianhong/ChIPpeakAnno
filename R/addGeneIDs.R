addGeneIDs<-function(annotatedPeak, orgAnn, IDs2Add=c("symbol"), 
    ...
    feature_ids <- switch(class(annotatedPeak),
                          RangedData=unique(annotatedPeak$feature),
    ...
    if(!missing(orgAnn)){
        ...
        is.installed <- function(orgpkg) is.element(orgpkg, installed.packages()[,1])
        if(!is.installed(orgAnn)){
            biocLite(pkgs=orgAnn, suppressUpdates=TRUE, suppressAutoUpdate=TRUE)
        }
        if(!library(orgAnn, character.only=TRUE, logical.return=TRUE)){
            if(!silence) message("No valid gene mapping package as argument orgAnn is passed in!")
        ...
        if(feature_id_type=="entrez_id"){
            ...
            m_ent <- do.call(rbind,lapply(gene_ids,function(.ele){
                ...
                if(!is.na(r[1])) cbind(rep(.ele,length(r)),r)
                    ...
                    if(!silence) message(paste("entrez id for '", .ele, "' not found\n", sep = ""))
                    c(.ele,NA)
                }
            }))
        ...
        for(IDtoAdd in IDs2Add){
            ...
            if(IDtoAdd!="entrez_id"){
                ...
                tryCatch(orgDB<-get(paste(orgAnn,toupper(IDtoAdd),sep="")),error = function(e){
                    if(!silence){
                        message(paste("The IDs2Add you input, \"", IDtoAdd, "\", is not supported!\n",sep=""))
                    }
                })
                if(is.null(orgDB)){
...
if(length(duplicated_ids)>0){
    ...
    m_ent.duplicated<-condenseMatrixByColnames(as.matrix(m_ent.duplicated),feature_id_type)
    m_ent<-m_ent[!(m_ent[,feature_id_type] %in% duplicated_ids),]
    m_ent<-rbind(m_ent,m_ent.duplicated)
}
if (class(annotatedPeak) %in% c("RangedData","GRanges")){
    ...
    m_ent<-m_ent[order(m_ent[,"orderid___"]),c(feature_id_type,IDs2Add)]
    if(class(annotatedPeak)=="RangedData"){
        for(IDtoAdd in IDs2Add){
            annotatedPeak$tobeAdd<-m_ent[,IDtoAdd]
