getAllPeakSequence <-
  function(myPeakList, upstream=200L, downstream=upstream, genome, AnnotationData)
  {
    ...
    if (class(genome)== "BSgenome")
    {
      ...
      width(myPeakList) <- width + as.numeric(upstream) + as.numeric(downstream) + start
      strand[strand!="-"] <- "+"
      ...
      ends <- unlist(apply(cbind(end(myPeakList), seqlengths(genome)[as.character(seqnames(myPeakList))]), 1,min))
      end(myPeakList) <- unname(ifelse(is.na(ends), end(myPeakList), ends))
      seq <- getSeq(genome, myPeakList, as.character=TRUE)
      if(flagRD){
        myPeakList <- RangedData(IRanges(start=start(myPeakList.bk), 
                                         end = end(myPeakList.bk), 
                                         names=rownames(myPeakList.bk)), 
                                 space=space(myPeakList.bk), 
                                 upstream=rep(upstream,length(myPeakList.bk)), 
                                 downstream=rep(downstream,length(myPeakList.bk)), 
                                 sequence=seq,
                                 strand=strand(myPeakList.bk))
      }else{
    ...
    else if (class(genome) =="Mart")
    {
      if (missing(AnnotationData)) {
        ...
        message("Done querying biomart database, start annotating ....Better way would be calling getAnnotation before querying for sequence")
      }
      if (!inherits(AnnotationData, c("RangedData", "GRanges"))) {
        stop("AnnotationData needs to be RangedData or GRanges object. Better way would be calling getAnnotation.")
      }
      ...
      r4 = merge(r, myList3)
      if(flagRD) 
        RangedData(IRanges(start=r4$start, end = r4$end, names =as.character(r4$name)), 
                   space=as.character(r4$space), upstream=rep(upstream, dim(r4)[1]), 
                   downstream= rep(downstream.bk, dim(r4)[1]), sequence=unlist(r4$seq))
      else
        GRanges(seqnames=as.character(r4$space), 
                ranges=IRanges(start=r4$start, end = r4$end, names =as.character(r4$name)),
                strand="+",
                upstream=rep(upstream, dim(r4)[1]), downstream= rep(downstream.bk, dim(r4)[1]),
