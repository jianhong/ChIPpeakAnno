...
#' 
#' re<-GetResultsFromDiffBind(mcf7,"no",output.file.dir)
#' 
#' 
GetResultsFromDiffBind<-function(mcf7,Mergereplicates=c("yes","no"),output.file.dir){
  ...
  if(!dir.exists(paste0(output.file.dir,"Venn/")))
  {
    dir.create(paste0(output.file.dir,"Venn/"))
  }
  ...
  for(i in 1:length(Tissue2)){
    ...
    if(length(po)==2)
    {
      png(paste0(paste0(output.file.dir,"Venn/"),paste0(colnames(temp22$class)[po],collapse = "-vs-"),".png"))
  ...
  if(!dir.exists(paste0(output.file.dir,"common_peaks_bed/")))
  {
    dir.create(paste0(output.file.dir,"common_peaks_bed/"))
    dir.create(paste0(output.file.dir,"common_peaks_bed/ucsc/"))
    dir.create(paste0(output.file.dir,"common_peaks_bed/igv/"))
  }
  ...
  lapply(1:length(p.common),function(u,p.common,dd.hs,output.file.dir){
    ...
    write.table(df,file=paste0(paste0(output.file.dir,"common_peaks_bed/"),x_name,"_cp_with_header.bed"),col.names=TRUE,row.names = FALSE,quote=FALSE,sep="\t")

    write.table(df.str.3,file=paste0(paste0(paste0(output.file.dir,"common_peaks_bed/"),"ucsc/"),x_name,"_4_ucsc.bed"),col.names=FALSE,row.names = FALSE,quote=FALSE,sep="\t")

    write.table(df,file=paste0(paste0(output.file.dir,"common_peaks_bed/"),x_name,"_common_peaks.bed"),col.names=FALSE,row.names = FALSE,quote=FALSE,sep="\t")

    write.table(df,file=paste0(paste0(paste0(output.file.dir,"common_peaks_bed/"),"igv/"),x_name,"_4_igv.bed"),col.names=FALSE,row.names = FALSE,quote=FALSE,sep="\t")
