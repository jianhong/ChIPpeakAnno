...
orderPeakAndOutPutHuman <- function(peakAll,output.file.dir,output.file.name,outHeader= FALSE) {
...
getBoxplot4DiffRnaSeq <- function() {
  ...
  getBoxPlot4FPKMOfSubsetPeaks1(YYY.ES1,rna.seq.data.ES1.rpkm,output.file.dir)
}
...
getBedFiles4 <- function(input.file.dir,select.index) {
  
  input.file.dir <- "/Users/aiminyan/Aimin/DropboxUmass/Aimin/Project/umw_junhao_mao"
  
  file.name.bedgraph <- list.files(input.file.dir,pattern="*.bed$",all.files = TRUE,full.names = TRUE,recursive = TRUE,include.dirs = TRUE)
  
  select.index <- c(1,2,3,4,5,6,7)
  ...
  names(bed.in) <- tools::file_path_sans_ext(basename(file.name.bedgraph[select.index]))
  bed.in
  
  peak.index <- c(1,2)
  name <- c("foxm1-rep1","foxm1-rep2")
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  peak.index <- c(3,4)
  name <- c("nf1c-rep1","nf1c-rep2")
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  peak.index <- c(1,2,5,6)
  name <- c("foxm1-rep1","foxm1-rep2","tead4-rep1","tead4-rep2")
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  peak.index <- c(3,4,5,6)
  name <- c("nf1c-rep1","nf1c-rep2","tead4-rep1","tead4-rep2")
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  ENCFF001UQJ.only <- GenomicRanges::setdiff(bed.in[[1]],bed.in[[2]])
  
  ENCFF001UQJ.and.ENCFF001UQK <- GenomicRanges::intersect(bed.in[[1]],bed.in[[2]])
  
  ENCFF001UQK.only <- GenomicRanges::setdiff(bed.in[[2]],bed.in[[1]])
  
  output.file.dir <- "/Users/aiminyan/Aimin/DropboxUmass/Aimin/Project/umw_junhao_mao/PeakOverlap"
  
  peak.index <- c(1,2,3)
  name <- c("SK-N-SH-rep1","SK-N-SH-rep2","BE2")
  
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  peak.index <- c(1,2)
  name <- c("SK-N-SH-rep1","SK-N-SH-rep2")
  ...
  X.U.y <- GenomicRanges::union(x, y,ignore.strand=FALSE)
  
  ENCFF001UQJ.and.BE2 <- GenomicRanges::intersect(bed.in[[5]],bed.in[[7]])
  ENCFF001UQJ.U.BE2 <- GenomicRanges::union(bed.in[[5]],bed.in[[7]])
  sum(width(ENCFF001UQJ.and.BE2))/sum(width(ENCFF001UQJ.U.BE2))
  
  ENCFF001UQJ.and.BE2 <- GenomicRanges::intersect(bed.in[[6]],bed.in[[7]])
  ENCFF001UQJ.U.BE2 <- GenomicRanges::union(bed.in[[6]],bed.in[[7]])
  sum(width(ENCFF001UQJ.and.BE2))/sum(width(ENCFF001UQJ.U.BE2))
  
  peaks=read.table("/Users/aiminyan/Aimin/DropboxUmass/Aimin/Project/umw_junhao_mao/paper/Tead4.SK-N-SH.common8726_jianhong.bed")
  
  colnames(peaks)[1:3]= c("chr","start","end")
  peaks=toGRanges(peaks)
  peaks
  
  bed.in["jianhong"] <- peaks
  
  ENCFF001UQJ.and.ENCFF001UQK <- GenomicRanges::intersect(bed.in[[5]],bed.in[[6]])
  
  ENCFF001UQJ.and.ENCFF001UQK.and.BE2 <- GenomicRanges::intersect(ENCFF001UQJ.and.ENCFF001UQK,bed.in[[7]])
  
  bed.in["SKNSH.and.BE2"] <- ENCFF001UQJ.and.ENCFF001UQK.and.BE2
  
  peak.index <- c(8,9)
  name <- c("jianhong","SKNSH.and.BE2")
  
  getCount4Venn(bed.in,peak.index,name,output.file.dir)
  
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[9]]),output.file.dir,"SKNSH_2_reps_and_BE2",outHeader= FALSE) 
  
  sum(width(GenomicRanges::intersect(bed.in[[8]],bed.in[[9]])))/sum(width(GenomicRanges::union(bed.in[[8]],bed.in[[9]])))
  
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[9]]),output.file.dir,"SKNSH_2_reps_and_BE2",outHeader= T) 
  
  
  ENCFF000QXW.and.ENCFF000QXX <- GenomicRanges::intersect(bed.in[[1]],bed.in[[2]])
  ENCFF000QXW.and.ENCFF000QXX.and.BE2 <- GenomicRanges::intersect(ENCFF000QXW.and.ENCFF000QXX,bed.in[[7]])
  bed.in["Foxm1.and.BE2"] <- ENCFF000QXW.and.ENCFF000QXX.and.BE2
  
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[10]]),output.file.dir,"Foxm1_2_reps_and_BE2",outHeader= F)
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[10]]),output.file.dir,"Foxm1_2_reps_and_BE2",outHeader= T) 
  
  ENCFF000QZS.and.ENCFF000QZT <- GenomicRanges::intersect(bed.in[[3]],bed.in[[4]])
  ENCFF000QZS.and.ENCFF000QZT.and.BE2 <- GenomicRanges::intersect(ENCFF000QZS.and.ENCFF000QZT,bed.in[[7]])
  bed.in["nf1c.and.BE2"] <- ENCFF000QZS.and.ENCFF000QZT.and.BE2
  
  unique(as.character(seqnames(bed.in[[7]])@values))
  unique(as.character(seqnames(ENCFF000QZS.and.ENCFF000QZT)@values))
  
  unique(as.character(seqnames(ENCFF000QXW.and.ENCFF000QXX)@values))
  
  ENCFF001UQJ.and.ENCFF001UQK
  unique(as.character(seqnames(ENCFF001UQJ.and.ENCFF001UQK)@values))
  
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[11]]),output.file.dir,"nf1c_2_reps_and_BE2",outHeader= F)
  orderPeakAndOutPutHuman(as.data.frame(bed.in[[11]]),output.file.dir,"nf1c_2_reps_and_BE2",outHeader= T) 
  

  genome(ENCFF001UQJ.and.ENCFF001UQK)
  genome(ENCFF000QZS.and.ENCFF000QZT)
  genome(ENCFF000QXW.and.ENCFF000QXX)
  
  genome(bed.in[[7]])
  
  bed.in[[9]][which(seqnames(bed.in[[9]])=="chr20")]
  ENCFF001UQJ.and.ENCFF001UQK[which(seqnames(ENCFF001UQJ.and.ENCFF001UQK)=="chrX")]
  ENCFF001UQJ.and.ENCFF001UQK[which(seqnames(ENCFF001UQJ.and.ENCFF001UQK)=="chrY")]
...
# Paul's RNA-Seq data
# mef
# input.file <- "~/Aimin/DropboxUmass/Aimin/Project/umw_paul_kaufman/mef/count.txt"
# toBeReplace <- c("X.project.umw_paul_kaufman.Aimin.mef.Alignment.","_rmDu_sorted.bam")
# sampleIndex <- c(8:19)
# output.file.dir <- "~/Aimin/DropboxUmass/Aimin/Project/umw_paul_kaufman/mef"
# x_name <- "mef"
# fromCount2FPKM(input.file,toBeReplace,sampleIndex,output.file.dir,x_name)

# SamllMEF
# input.file <- "~/Aimin/DropboxUmass/Aimin/Project/umw_paul_kaufman/SmallMEF/count.txt"
# toBeReplace <- c("X.project.umw_paul_kaufman.Aimin.SmallMEF.Alignment.","_rmDu_sorted.bam")
# 
# sampleIndex <- c(8:12)
# output.file.dir <- "~/Aimin/DropboxUmass/Aimin/Project/umw_paul_kaufman/SmallMEF"
# x_name <- "SmallMEF"
# 
# fromCount2FPKM(input.file,toBeReplace,sampleIndex,output.file.dir,x_name)

fromCount2FPKM <- function(input.file,toBeReplace,sampleIndex,output.file.dir,x_name) {
  ...
  count.data.1 <- data.frame(count.data,ensembl_gene_id = genes)
  G_list <- getBM(filters= "ensembl_gene_id", attributes= c("ensembl_gene_id","mgi_symbol"),values=count.data.1$ensembl_gene_id,mart= mart)
  count.data.2 <- merge(count.data.1,G_list,by="ensembl_gene_id",sort = F)
  count.data.2[which(count.data.2$mgi_symbol %in% ""),]$mgi_symbol <- paste0(count.data.2[which(count.data.2$mgi_symbol %in% ""),]$ensembl_gene_id,"-No_match")
  
  colnames(count.data.2) <- gsub(toBeReplace[1],"",colnames(count.data.2))
  colnames(count.data.2) <- gsub(toBeReplace[2],"",colnames(count.data.2))
  ...
  count.data.2.rpkm <- apply(count.data.2[,sampleIndex],2,countToFpkm,count.data.2[,7])
  
  count.and.rpkm <- cbind(count.data.2,count.data.2.rpkm)
  
  write.table(count.and.rpkm,file = file.path(output.file.dir,paste0(x_name,"_count_rpkm",".csv")),
              append = FALSE, quote = F, sep = ",",eol = "\n", na = "NA", dec = ".", row.names = F,col.names = T)
...
# /Users/aiminyan/Aimin/DropboxUmass/NADfinder/Aimin/F121_9_RNA_seq_data/GSM1621026_WT_fpkm.csv
  ...
  YYY1.GSE90894.mef.XL_NAD <- YYY1.GSE90894.mef[YYY1.GSE90894.mef$SetName=="XL_NAD",]
  
  YYY1.XL_NAD <- YYY1[YYY1$SetName=="XL_NAD",]
  
  YYY.ES1.XL_NAD <- YYY.ES1[YYY.ES1$SetName=="XL_NAD",]
  
  YYY.ESCs.XL_NAD <- YYY.ESCs[YYY.ESCs$SetName=="XL_NAD",]
  
  YYY.Aizhan.XL_NAD <- YYY.Aizhan[YYY.Aizhan$SetName=="XL_NAD",]
