BED2RangedData <- function(data.BED,header=FALSE, ...)
{
	...
	{
		stop("No valid data.BED passed in, which is a data frame or file as BED format with at least 3 fields in the order of: chromosome, start and end. Optional fields are name, score and strand etc. Please refer to http://genome.ucsc.edu/FAQ/FAQformat#format1 for details.  If it is a file, please make sure the file path is correct!")
	}
	if (header == TRUE)
	{  
		myPeak = data.BED[-1,]
	...
	{		
		...
		{
			stop("only 1, -1, + and - are acceptable for strand info. Please double check you BED data")
		}
		strand[strand== "1"] = "+"
		strand[strand== "-1"] = "-"
	}
