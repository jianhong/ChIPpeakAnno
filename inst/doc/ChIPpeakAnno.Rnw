...
To annotate the peaks with other genomic feature, you will need to call function \Rfunction{getAnnotation} with featureType, e.g., ``Exon" for finding the nearest exon, and ``miRNA" for finding the nearest miRNA. Please refer to \Rfunction{getAnnotation} function for more details. 
...
Once the overlapping peaks are obtained, Venn Diagram can be generated by the following function call with p-value that indicates whether the extent of overlapping is significant.
\begin{scriptsize}
<<>>=
makeVennDiagram(peaks1, peaks2, overlappingPeaks, totalTest=100, NameOfPeaks1 = "TF1", NameOfPeaks2 = "TF2", cex = 1, counts.col = "red")
@
\end{scriptsize}
