test_annotatePeakInBatch<-function(){
    load(system.file("extdata", "annotatedPeaks.2.10.0.rds", ## this file should be rewrote for fix the new format
    ...
    checkRes <- function(step, s){
        ...
        if(length(s$feature)>0){
            ...
            s <- as.data.frame(s)
            for(i in c("peak", "feature", "start_position", "end_position", "distancetoFeature", "shortestDistance")){
    ...
    #       +HHHHH
    #     +IIIIIII
    myPeak = RangedData(IRanges(start=c(2, 7, 10, 17), end=c(6, 8, 12, 17), names=letters[1:4]), space="1", strand="*")
    feature = RangedData(IRanges(start=c(1, 3, 6, 8, 2, 3, 1, 3, 1, 11, 13, 19, 18), end=c(2, 3, 7, 9, 6, 5, 5, 7, 7, 13, 15, 20, 20), names=LETTERS[1:13]), space="1", strand=c("-", "+", "+", "-", "+", "+", "-", "+", "+", "+", "-", "+", "+"))
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=1, output="upstream&inside"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 8)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 4)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 1)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=2, output="upstream&inside"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 8)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 5)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 3)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=1, output="upstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 6)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 0)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 1)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=2, output="upstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 6)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 1)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 3)
    #check upsteam&inside and inside&downstream
    #       aaaaabb ccc    d
    ...
    #       +HHHHH
    #     +IIIIIII
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=1, output="inside&downstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 8)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 5)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 0)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=2, output="inside&downstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 9)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 6)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 0)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=1, output="downstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 2)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 5)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 1)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 0)
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=2, output="downstream"))
    checkEquals(sum(annotation$peak=="a"& !is.na(annotation$feature)), 3)
    checkEquals(sum(annotation$peak=="b"& !is.na(annotation$feature)), 6)
    checkEquals(sum(annotation$peak=="c"& !is.na(annotation$feature)), 1)
    checkEquals(sum(annotation$peak=="d"& !is.na(annotation$feature)), 0)
    ...
    #       CC    DD
    #
    myPeak <- RangedData(IRanges(start=c(7, 15), end=c(8, 16), names=c("a","b")), space="1", strand="*")
    feature <- RangedData(IRanges(start=c(1, 4, 4, 10), end=c(14, 11, 5, 11), names=c("A", "B", "C", "D")), space="1", strand="+")
    suppressWarnings(annotation <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=1, output="upstreamORdownstream"))
    suppressWarnings(annotation1 <- annotatePeakInBatch(myPeak, AnnotationData=feature, maxgap=2, output="upstreamORdownstream"))
    checkEquals(nrow(annotation), 2)
    checkEquals(nrow(annotation1), 3)
    ...
    #checkRes(annotation.step14,annotation.s14) , difference: old 34 ENSG00000246463, new 34 ENSG00000181894
...
## there is a bug for 2.10.0 when calculating the distancetoFeature for negative strand.
    ...
    if(interactive()){
        ...
        output <- c("nearestLocation", "overlapping", "both", "shortestDistance", "upstream&inside", "inside&downstream", "upstream", "downstream", "upstreamORdownstream", "inside") ##, "shortestDistance" is not in version 2.10.0
...
gaps.anno <- annotatePeakInBatch(gaps, AnnotationData=exons, output="inside")
checkTrue(all(is.na(gaps.anno$feature)))
start(gaps) <- start(gaps)+2
end(gaps) <- end(gaps)-2
gaps.anno <- annotatePeakInBatch(exons, AnnotationData=gaps, output="overlapping", maxgap=0, ignore.strand = FALSE)
gaps.anno.1 <- gaps.anno[!is.na(gaps.anno$feature)]
checkEquals(length(gaps.anno.1), 0)
gaps.anno <- annotatePeakInBatch(exons, AnnotationData=gaps, output="overlapping", maxgap=3, ignore.strand = FALSE)
gaps.anno.2 <- gaps.anno[!is.na(gaps.anno$feature)]
checkEquals(length(gaps.anno.2), 2*length(exons))
exons.anno <- annotatePeakInBatch(exons, AnnotationData=exons, output="inside")
checkEquals(exons.anno$peak, exons.anno$feature)
exons.anno <- annotatePeakInBatch(exons, AnnotationData=exons, output="nearestLocation")
checkEquals(exons.anno$peak, exons.anno$feature)
threeUTRs.anno <- annotatePeakInBatch(threeUTRs, AnnotationData=exons, output="downstream", maxgap=1000)
fiveUTRs.anno <- annotatePeakInBatch(fiveUTRs, AnnotationData=exons, output="upstream", maxgap=1000)
threeUTRs.anno <- annotatePeakInBatch(threeUTRs, AnnotationData=exons, output="inside&downstream")
fiveUTRs.anno <- annotatePeakInBatch(fiveUTRs, AnnotationData=exons, output="upstream&inside")
checkEquals(threeUTRs.anno$peak, threeUTRs.anno$feature)
checkEquals(fiveUTRs.anno$peak, fiveUTRs.anno$feature)
}