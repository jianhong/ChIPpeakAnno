...
author:
- name: Jianhong Ou
  affiliation: Morgridge Institute for Research, Wisconsin, USA
- name: Kai Hu
...
```{r}
names(txdb.hg38.transcript) <- txdb.hg38.transcript$tx_name
```
...
```{r tryBiomartAnno,eval=FALSE}
# try biomaRt if it works
library(biomaRt)
...
```{r}
# If biomaRt server stopped response, 
# try to use local package org.Hs.eg.db.
library(org.Hs.eg.db)
res <- addGeneIDs(res, 
                  orgAnn = TxDb.Hsapiens.UCSC.hg38.knownGene,
                  feature_id_type = "ensembl_transcript_id",
                  IDs2Add = "GENEID")
res$symbol[!is.na(res$GENEID)] <- 
    xget(res$GENEID[!is.na(res$GENEID)], org.Hs.egSYMBOL, output = 'first')
tail(res, n = 2)
```
