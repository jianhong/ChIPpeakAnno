...
2 Generate annotation data with `annoGR`
...
peaks[1:2]
```
## Step 2: Prepare annotation data with `annoGR`
```{r annotationData}
library(EnsDb.Hsapiens.v75)
...
This section demonstrates how to annotate the same peak data as in quick start 1 using a new annotation based on **TxDb** with `annoGR`.
...
seqlevelsStyle(peaks) <- seqlevelsStyle(annoData)
```
The same `annotatePeakInBatch` function is used to annotate the peaks using annotation data just created.
```{r}
anno <- annotatePeakInBatch(peaks, AnnotationData=annoData, 
                  output="bindingRegion_startSite", 
...
anno <- annotatePeakInBatch(peaks, AnnotationData=annoData, 
                  ouput="bindingRegion_bothSidesNSS", 
                  bindingRegion=c(-5000, 5000))
anno$symbol <- xget(anno$feature, org.Hs.egSYMBOL)
anno[names(anno)=="peak12725"]
```
